...
class SesionCajaHostal(models.Model):
    """Caja independiente del hostal. No comparte datos con el restaurante."""
    ...
    monto_final_fisico = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    diferencia = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    estado = models.BooleanField(default=True, verbose_name="¿Caja Abierta?")  # True = Abierta

    def __str__(self):
        return f"Caja Hostal {self.usuario} - {self.fecha_apertura.strftime('%d/%m/%Y %H:%M')}"
